...
const ClubPricing = () => {
  ...
  const [ClubID, setClubID] = useState("random ID again");
  const [clubName, setClubName] = useState("new club");
  const [userId, setuserId] = useState("some user id");
  const [username, setUsername] = useState("new username");
  const [Mobile_number, setMobile_number] = useState("98765");
  const [price, setPrice] = useState("11111111");
  ...
  async function callData() {
    try {
      ...
      const user_name = await Axios.get("http://localhost:5005/api/users/aryan");
      
      const userdata = user_name.data[0];
      console.log(user_name.data[0]);
      setUsername(userdata.username);
      setClubName(userdata.username);
      setClubID(userdata._id);
